...
from utils.supabase_mirror import mirror_goal
...
class Goals(commands.Cog):
    ...
    async def set_goal(
        ...
        if previous_goal:
            await mirror_goal(self.bot, await self.bot.db.get_goal(previous_goal["id"]))
        await mirror_goal(self.bot, await self.bot.db.get_goal(goal["id"]))
